...
export const UNKNOWN_AVATAR_ID_FORMAT = '~0x/123,joe';
export const UNKNOWN_TOKEN_ID_FORMAT = '~0x/123,joe';
export const DEFAULT_AVATAR_FALLBACK_TYPE = 'jazzicon';
export const DEFAULT_TOKEN_FALLBACK_TYPE = 'identicon';
...
export function expectHeaders(response: AxiosResponse, expectedHeaders: AxiosResponseHeaders) {
  Object.entries(expectedHeaders).forEach(([key, value]) => {
    expect(response.headers[key]).toBe(value);
  });
}

export function getImageResponse(path: string) {
  return axios.get(`${HOST}${path}`, { responseType: 'arraybuffer' });
}

export function getAvatarResponse(identifier: string) {
  return getImageResponse(`/avatar/${identifier}`);
}

export function getTokenResponse(identifier: string) {
  return getImageResponse(`/token/${identifier}`);
}
